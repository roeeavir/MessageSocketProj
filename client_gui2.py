...
connect_btn = tkinter.Button(n, text='Set name', command=lambda: client2.set_connection(client_host, client_port,
                                                                                       client_name, n))
...
send_bt = tkinter.Button(m, text='Send', height=3, width=9, command=lambda: client2.send(client_message_box, m))
...
receive_thread = Thread(target=lambda: client2.receive(chat))
receive_thread.start()

m.protocol("WM_DELETE_WINDOW", lambda: client2.on_closing(m))
